...
**SciTaRC** is an expert-authored benchmark designed to evaluate composite question-answering over raw LaTeX tables from arXiv papers. By pairing multi-step tasks with structured pseudo-code plans and 7 diagnostic complexity metrics, SciTaRC decouples reasoning strategy from execution—revealing that even frontier models suffer from a fundamental **execution bottleneck** on empirical scientific data.
...
```bash
git clone [https://github.com/JHU-CLSP/SciTaRC.git](https://github.com/JHU-CLSP/SciTaRC.git)
cd SciTaRC
...
* `plan` *(string)*: The expert-authored pseudo-code blueprint (e.g., `SELECT`, `LOOP`, `COMPUTE`, `IF...ELSE`, `RETURN`).
